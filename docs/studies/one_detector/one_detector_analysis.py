...
FMASK = jnp.array((FREQ >= BAND[0]) & (FREQ <= BAND[1]))
...
def fetch_strain(trigger_time, duration, cache_file: Path):
...
def inner_product(x: np.ndarray, y: np.ndarray, psd_vals: np.ndarray, fs: float) -> float:
    ...
    Xk = (np.fft.rfft(x * WINDOW) / C)[FMASK]
    Yk = (np.fft.rfft(y * WINDOW) / C)[FMASK]
    Sk = psd_vals[FMASK]
...
def snr_from_excess_power(x: np.ndarray, psd_vals: np.ndarray, fs=FS) -> float:
    ...
    Xf = (np.abs(np.fft.rfft(x * WINDOW) / C))[FMASK]
    Sf = psd_vals[FMASK]
...
def inject_signal_into_data(analysis_segment: TimeSeries, psd: FrequencySeries, kind: str, target_snr: float):
    ...
    injection = gen_waveform(z, model=model)

    d = analysis_segment.value + injection
    snr = snr_from_excess_power(d, psd.value)

    # now we need to scale the injection to reach the target SNR
    scale = target_snr / snr
    injection = injection * scale
    analysis_segment = TimeSeries(analysis_segment.value + injection, sample_rate=analysis_segment.sample_rate)
...
def plot_data(
        ...
        posterior_signal: Optional[np.ndarray] = [],
        posterior_blip: Optional[np.ndarray] = [],
    ...
    def _plot_post(label, color, series):
        if series is None or len(series) == 0:
            return
        f_p, Pxx_p = periodogram(series * WINDOW, fs=SAMPLE_RATE, axis=-1)
...
def build_whittle_context(data_ts: TimeSeries, injected_wf: np.ndarray, psd: FrequencySeries, fs=FS, analysis_len=N, ):
    data_ts = np.asarray(data_ts.value) * WINDOW
    injected_wf = np.asarray(injected_wf) * WINDOW


    _, Pxx_psd = psd.frequencies.value, psd.value


    D_full = np.fft.rfft(data_ts) / C
    f_k = FREQ[FMASK]
    D_k = D_full[FMASK]
    PSD_k = Pxx_psd[FMASK]
    y_obs = np.stack([D_k.real, D_k.imag], axis=-1)
    sigma_comp = np.sqrt(PSD_k / 2.0)

    lnz_noise = -np.sum((np.abs(D_k) ** 2) / PSD_k + np.log(np.pi * PSD_k))

    # for plotting...
    # filter a bit more to reduce variance??
    _, Pxx_data = periodogram(data_ts, fs=fs)
    _, Pxx_sig = periodogram(injected_wf, fs=fs)
...
def run_model(model_fn, y, name, rng_key, num_warmup=1000, num_samples=1000):
    nuts = NUTS(model_fn)
    ...
    print(f"{name} model sampling complete. ESS (bulk):")
    print(ess)
...
def posterior_draw_time_series(samples, ctx, which="signal", n_draws=20):
    ...
    for i in range(n):
        ...
        Hfull_scaled = Hfull_scaled.at[FMASK].set(Hk_scaled)
...
def main(detector=DEFAULT_DETECTOR, trigger_time=DEFAULT_TRIGGER_TIME, inject_kind=INJECT_KIND, seed=0,
    ...
    strain = fetch_strain(trigger_time, DURATION, cache_file=cache_file)
    ...
    if inject_kind == "noise":
        injection = np.zeros_like(ANALYSIS_LEN)
    else:
    ...
    print(f"snr = {snr_excess:.2f}")
    plot_data(ctx, inject_kind, injection_snr=snr_excess, outpath=outdir / "frequency_summary.png")
    ...
    ts_gli = posterior_draw_time_series(res_gli, ctx, "glitch", n_draws=10)
