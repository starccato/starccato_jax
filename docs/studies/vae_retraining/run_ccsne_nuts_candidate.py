"""Run a multistart-MAP plus four-chain NUTS check for a CCSNE VAE.

This script uses ``starccato_lvk`` and an injection manifest produced by the
figure/validation workflow. Launch it from that repository's environment while
pointing ``PYTHONPATH`` at the local ``starccato_jax`` source. It intentionally
does not run nested sampling.
"""
...
def main() -> None:
    ...
    parser.add_argument("--scenario", default="inj_ccsn")
    ...
    bundle = Path(manifest["bundles"][args.scenario][args.detector])
    prepared = prepare_multi_detector_data(
        [args.detector],
        bundle_paths={args.detector: str(bundle)},
        flow=args.flow,
        fmax=args.fmax,
    )
    reference = prepared.detector_data[args.detector]
    waveform = StarccatoJimWaveform(
        model=StarccatoVAE(str(args.model_dir)),
        sample_rate=1.0 / reference.dt,
        window=prepared.window,
    )
    fixed = dict(DEFAULT_EXTRINSICS)
    fixed.update(manifest["sky"])
    fixed.update(gmst=prepared.gmst, trigger_time=prepared.trigger_time)
