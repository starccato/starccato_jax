...
def cyclical_annealing_beta(n_epoch, start=0.0, stop=1.0, n_cycle=4, ratio=0.5):
    """
    ...
    if n_cycle > 0:
        ...
        for c in range(n_cycle):
            ...
            for i in range(int(period * ratio)):  # Annealing phase
                idx = int(i + c * period)
                if idx < n_epoch:
                    beta_schedule[idx] = 1.0 / (1.0 + np.exp(- (v * 12. - 6.)))
                v += step
...
def aggregate_metrics(metrics_list: List[TrainValMetrics]) -> TrainValMetrics:
    ...
    return TrainValMetrics(
        train_metrics=Losses(
            reconstruction_loss=[m.train_metrics.reconstruction_loss for m in metrics_list],
            kl_divergence=[m.train_metrics.kl_divergence for m in metrics_list],
            loss=[m.train_metrics.loss for m in metrics_list],
            beta=[m.train_metrics.beta for m in metrics_list],
        ),
        val_metrics=Losses(
            reconstruction_loss=[m.val_metrics.reconstruction_loss for m in metrics_list],
...
def compute_metrics(state, x, rng, model, validation_x, beta) -> TrainValMetrics:
    return TrainValMetrics(
