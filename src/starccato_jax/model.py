...
def _reparameterize(rng: PRNGKey, mean: jnp.ndarray, logvar: jnp.ndarray) -> jnp.ndarray:
...
def generate(model_data: ModelData, z: jnp.ndarray = None, rng: PRNGKey = None) -> jnp.ndarray:
    rng = rng if rng is not None else PRNGKey(0)
    z = jax.random.normal(rng, shape=(1, model_data.latent_dim)) if z is None else z
    model = VAE(model_data.latent_dim)
    return model.apply({'params': model_data.params}, z, method=model.generate)


def reconstruct(x: jnp.ndarray, model_data: ModelData, rng: PRNGKey = None, n_reps: int = 1) -> jnp.ndarray:
    rng = rng if rng is not None else PRNGKey(0)
